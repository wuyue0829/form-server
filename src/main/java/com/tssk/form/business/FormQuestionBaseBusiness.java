...
@Business(name = "FormQuestionBaseBusiness")
public class FormQuestionBaseBusiness {
	...
	public BusinessResponse queryQuestionOne(BusinessRequest request) {
		...
		String sql = "select * from  form_question_base where id = ? and app_code = ? and id = ?";
		List<Map<String, Object>> list = getSqlRunner().listBySql(request, sql, formId, appCode, questionId);
		sql = "select * from  form_question_grid where id = ? and app_code = ? and question_id = ?";
	...
	public BusinessResponse saveQuestionOne(BusinessRequest request) {
		...
		String sql = "delete from form_question_base where app_code = ? and form_id = ? and id = ?";
		getSqlRunner().execBySql(request, sql, appCode, formId, questionId);
		sql = "delete from form_question_grid where app_code = ? and form_id = ? and question_id = ?";
		getSqlRunner().execBySql(request, sql, appCode, formId, questionId);
		...
		for (QuestionBean questionBean : beans) {
			getSqlRunner().insert(request, "form_question_base", questionBean.getBase());
			List<Map<String, Object>> list = questionBean.getGrids();
			for (Map<String, Object> tempMap : list) {
				getSqlRunner().insert(request, "form_question_grid", tempMap);
			}
		}
		return response;
	...
	public BusinessResponse saveQuestionAll(BusinessRequest request) {
		...
		String sql = "update form_main_base set form_name=?,remarks = ?,end_word = ?,url_pic = ?,url_base64 = ?,url = ? where app_code = ? and id = ?";
		getSqlRunner().execBySql(request, sql, formTitle, remarks, endWord,
				"http://qr.liantu.com/api.php?text=http://192.168.1.175:82/#/test?id="+ formId,
				DownloadSavaDB.downloadPicture("http://qr.liantu.com/api.php?text=http://192.168.1.175:82/#/test?id=" + formId),
				"http://192.168.1.175:82/#/test?id=" + formId,appCode, formId);

		List<Object> dataMap = FwUtils.getJsonService().toList(data);
		sql = "delete from form_question_base where app_code = ? and form_id = ?";
		getSqlRunner().execBySql(request, sql, appCode, formId);
		sql = "delete from form_question_grid where app_code = ? and form_id = ?";
		getSqlRunner().execBySql(request, sql, appCode, formId);
		List<QuestionBean> beans = new ArrayList<QuestionBean>();
	...
	@BusinessMethod
	@Transactional(name = FormConsts.DB_NAME)
	public BusinessResponse saveRequirementsMetQuestionAll(BusinessRequest request) {
		BusinessResponse response = new BusinessResponse();
		String appCode = BusinessUtils.getCurrentAppCode(request);
		long formId = LangUtils.parseLong(request.getData("formId"));
		String data = StringUtils.trimNull(request.getData("data"));
		...
		String remarks = StringUtils.trimNull(request.getData("remarks"));
		String endWord = StringUtils.trimNull(request.getData("endWord"));
		//更新问卷名称，后续备注，图片地址和问卷分享地址
		String sql = "update form_main_base set form_name=?,remarks = ?,end_word = ?,url_pic = ?,url = ? ,url_base64 = ? where app_code = ? and id = ?";
		getSqlRunner().execBySql(request, sql, formTitle, remarks, endWord,
				"http://qr.liantu.com/api.php?text=http://192.168.1.175:82/#/test?id="+ formId
				,"http://192.168.1.175:82/#/test?id=" + formId,
				DownloadSavaDB.downloadPicture("http://qr.liantu.com/api.php?text=http://192.168.1.175:82/#/test?id=" + formId),
				appCode, formId);
		//删除所有该问卷下的题目
		List<Object> dataMap = FwUtils.getJsonService().toList(data);
		...
		for (Object object : dataMap) {
			...
			if (type.equals(FormConsts.QUESITON_TYPE_BASE_TEXT)) {
				...
				oneMap.put("baseType", type);
				QuestionBean bean = GradeRadioUtils.unwraptype(appCode, formId, oneMap,1);
				beans.add(bean);
			} else if (type.equals(FormConsts.QUESITON_TYPE_GRADE_CHECKBOX)) {
				oneMap.put("baseType", type);
				QuestionBean bean = GradeCheckboxUtils.unwraptype(appCode, formId, oneMap,1);
				beans.add(bean);
			} else if (type.equals(FormConsts.QUESITON_TYPE_UPLOAD_FILE)) {
				oneMap.put("baseType", type);
				QuestionBean bean = UploadFileUtils.unwrap(appCode, formId, oneMap);
	...
	@BusinessMethod
	@Transactional(name = FormConsts.DB_NAME)
	public BusinessResponse saveCourseQualityQuestionAll(BusinessRequest request) {
		BusinessResponse response = new BusinessResponse();
		String appCode = BusinessUtils.getCurrentAppCode(request);
		long formId = LangUtils.parseLong(request.getData("formId"));
		String data = StringUtils.trimNull(request.getData("data"));
		...
		for (Object object : dataMap) {
			...
			if (type.equals(FormConsts.QUESITON_TYPE_BASE_TEXT)) {
				...
				oneMap.put("baseType", type);
				QuestionBean bean = GradeRadioUtils.unwraptype(appCode, formId, oneMap, 2);
				beans.add(bean);
			} else if (type.equals(FormConsts.QUESITON_TYPE_GRADE_CHECKBOX)) {
				oneMap.put("baseType", type);
				QuestionBean bean = GradeCheckboxUtils.unwraptype(appCode, formId, oneMap,2);
				beans.add(bean);
			} else if (type.equals(FormConsts.QUESITON_TYPE_UPLOAD_FILE)) {
				oneMap.put("baseType", type);
				QuestionBean bean = UploadFileUtils.unwrap(appCode, formId, oneMap);
