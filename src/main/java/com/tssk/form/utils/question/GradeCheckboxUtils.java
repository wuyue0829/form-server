...
public class GradeCheckboxUtils {
	...
	public static Map<String, Object> wrapResult(QuestionBean bean, BusinessRequest request) {
		...
		for (Map<String, Object> map : items) {
			...
			if ("item".equals(propKey)) {
				...
				tempMap.put("text", propValue1);
				tempMap.put("scores", Double.parseDouble(propValue2));
				tempMap.put("number",list.size());
				NumberFormat format = NumberFormat.getIntegerInstance();
				format.setMaximumFractionDigits(2);//设置保留几位小数
				if(list.size() < 1){
					tempMap.put("proportion",Double.parseDouble(format.format(0)));
				}else{
					tempMap.put("proportion",Double.parseDouble(format.format(((double) list.size()*100/(double)list1.size()))));
				}
				itemList.add(tempMap);
			}
		}
